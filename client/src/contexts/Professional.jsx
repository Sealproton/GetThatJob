...
function ProProvider(props) {
  ...
  const getJobs = async (input) => {
    ...
    try {
      ...
      const result = await axios.get(
        `${import.meta.env.VITE_SERVER_URL}/pro?${params.toString()}`
      );
  ...
  const getPopularJob = async () => {
    try {
      const popularJob = await axios.get(
        `${import.meta.env.VITE_SERVER_URL}/pro/job/popular`
      );
      let popularSearch = [];
  ...
  const getSingleJob = async (id) => {
    return axios.get(`${import.meta.env.VITE_SERVER_URL}/pro/job/${id}`);
  };
  //JobHeader,JobCard
  const getJobFollowStatus = async (userID, job_id) => {
    try {
      const jobFollowStatus = await axios.get(
        `${import.meta.env.VITE_SERVER_URL}/pro/follow/job`,
        {
  ...
  const updateJobFollowStatus = async (data) => {
    ...
    try {
      axios.put(`${import.meta.env.VITE_SERVER_URL}/pro/follow/job`, { data });
    } catch (error) {
      console.log(error);
    }
  };
  const addJobProfessionalData = async (data) => {
    try {
      await axios.post(`${import.meta.env.VITE_SERVER_URL}/pro/follow/job`, {
        data,
      });
    } catch (error) {
      console.log(error);
