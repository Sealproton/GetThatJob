  ...
  async function handleClickCloseJob(data) {
    try {
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_SERVER_URL}/recruiter-display/job-status`,
        data
      );
...
export function ClosedJobButton() {
  return (
    <button
      className='flex flex-row items-center justify-center bg-[#E1E2E1] w-[154.67px] h-[53.36px] gap-2 text-[#8E8E8E] rounded-[21.33px] text-[15px] duration-1000'
      disabled
    >
      <svg
        width='24'
...
export function RadioCandidate(props) {
  const { candidateFilterState, setCandidateFilterState } =
    useRecruiterContext();
  return (
    <div className='pt-[21.33px] text-[#616161] text-[15px] '>
...
export function MarKAsStartedButton(props) {
  ...
  async function handleClickMarkStatus(data) {
    try {
      setIsLoading(true);
      const response = await axios.put(
        `${import.meta.env.VITE_SERVER_URL}/recruiter-display/candidate-status`,
        data
      );
      await props.refreshData();
...
export function FinishedButton() {
  return (
    <button
      className='text-[#8E8E8E] text-[15px] w-[240px] h-[53.33px] border-[1px] bg-[#E1E2E1] rounded-[21.33px] duration-300'
      disabled
    >
      FINISHED
    </button>
